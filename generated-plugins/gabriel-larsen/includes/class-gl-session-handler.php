 ...
 * Implements the GL_Session abstract class.
 *
 * @class    GL_Session_Handler
 ...
 */
class GL_Session_Handler extends GL_Session {
	...
	public function __construct() {
		...
		$this->_cookie = apply_filters( 'gabriel_larsen_cookie', 'wp_gabriel_larsen_session_' . COOKIEHASH );
		$this->_table  = $wpdb->prefix . 'gabriel_larsen_sessions';
		...
		// Actions
		add_action( 'gabriel_larsen_cleanup_sessions', array( $this, 'cleanup_sessions' ), 10 );
	...
	public function set_session_expiration() {
		$this->_session_expiring   = time() + intval( apply_filters( 'gabriel_larsen_session_expiring', 60 * 60 * 47 ) ); // 47 Hours.
		$this->_session_expiration = time() + intval( apply_filters( 'gabriel_larsen_session_expiration', 60 * 60 * 48 ) ); // 48 Hours.
	}
	...
	private function get_cache_prefix() {
		return GL_Cache_Helper::get_cache_prefix( GL_SESSION_CACHE_GROUP );
	}
	...
	public function save_data() {
		...
		if ( $this->_dirty && $this->has_session() ) {
			...
			// Set cache
			wp_cache_set( $this->get_cache_prefix() . $this->_id, $this->_data, GL_SESSION_CACHE_GROUP, $this->_session_expiration - time() );
	...
	public function destroy_session() {
		// Clear cookie
		gabriel_larsen_setcookie( $this->_cookie, '', time() - YEAR_IN_SECONDS, apply_filters( 'gabriel_larsen_session_use_secure_cookie', false ) );
	...
	public function cleanup_sessions() {
		...
		if ( ! defined( 'WP_SETUP_CONFIG' ) && ! defined( 'WP_INSTALLING' ) ) {
			...
			// Invalidate cache
			GL_Cache_Helper::incr_cache_prefix( GL_SESSION_CACHE_GROUP );
		}
	}
	...
	public function get_session( $id, $default = false ) {
		...
		$value = wp_cache_get( $this->get_cache_prefix() . $id, GL_SESSION_CACHE_GROUP );
		...
		if ( false === $value ) {
			...
			wp_cache_add( $this->get_cache_prefix() . $id, $value, GL_SESSION_CACHE_GROUP, $this->_session_expiration - time() );
		}
	...
	public function delete_session( $id ) {
		...
		wp_cache_delete( $this->get_cache_prefix() . $id, GL_SESSION_CACHE_GROUP );
